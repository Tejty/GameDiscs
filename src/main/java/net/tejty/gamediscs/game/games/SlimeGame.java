...
import net.tejty.gamediscs.game.VecUtil;
...
public class SlimeGame extends Game {
    ...
    private Sprite snakeRenderer = new Sprite(Vec2.ZERO, new Vec2(TILE_SIZE, TILE_SIZE), BODY);
    private Sprite apple = new Sprite(Vec2.ZERO, new Vec2(TILE_SIZE, TILE_SIZE), APPLE);
    ...
    private void respawnApple() {
        ...
        while (valid == false) {
            ...
            for (int i = 0; i < snake.size(); i++) {
                if (VecUtil.is(snake.get(i), applePos)) {
                    valid = false;
                }
            }
